...
function runTheRegister() {
   ...
   try {
    // If the changeValue is less than 0, throw an error
    if (changeValue < 0) {
      throw new Error("Cash amount doesn't cover the bill");
    }

    //If no error, run these commands
    changeBox.value = formatCurrency(changeValue); // format the change as a currency
